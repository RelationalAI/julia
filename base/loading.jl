    ...
    try
        ...
        sv = try
            if ocachepath !== nothing
                ...
                ccall(:jl_restore_package_image_from_file, Any, (Cstring, Any, Cint, Cstring), ocachepath, depmods, false, pkg.name)
            else
    ...
    if root_module_exists(modkey)
        ...
        if loaded === nothing
            try
                ...
                modpath === nothing && error("Cannot locate source for $(repr("text/plain", modkey))")
...
@constprop :none function _require_search_from_serialized(pkg::PkgId, sourcepath::String, build_id::UInt128, stalecheck::Bool)
    assert_havelock(require_lock)
