...
{
    jl_taggedvalue_t *obj_astagged = jl_astaggedvalue(obj);
    jl_taggedvalue_t *obj_vtag = (jl_taggedvalue_t *)(obj_astagged->header & ~(uintptr_t)0xf);
    uintptr_t obj_vt = (uintptr_t)obj_vtag;
    if (obj_vt == (jl_datatype_tag << 4) ||
        obj_vt == (jl_unionall_tag << 4) ||
        obj_vt == (jl_uniontype_tag << 4) ||
        obj_vt == (jl_tvar_tag << 4) ||
        obj_vt == (jl_vararg_tag << 4)) {
        // Skip, since these wouldn't hit the object assert anyway
        return;
    }
    else if (obj_vt < jl_max_tags << 4) {
        // Skip, since these wouldn't hit the object assert anyway
        return;
    }
    else if (obj_vt == jl_buff_tag) {
        // Skip, since these wouldn't hit the object assert anyway
        return;
    }
    else if (obj_vt == jl_string_tag) {
        // Skip, since these wouldn't hit the object assert anyway
        return;
    }
    else if (obj_vt == jl_simplevector_tag) {
        // Skip, since these wouldn't hit the object assert anyway
        return;
    }
    if (__unlikely(!jl_is_datatype((jl_datatype_t *)obj_vt) || ((jl_datatype_t *)obj_vt)->smalltag)) {
        jl_safe_printf("Invalid object (probable corruption)\n");
        jl_print_task_backtraces(1);
        abort();
    }
}

void gc_assert_obj_validity(jl_value_t *parent, jl_value_t *child) JL_NOTSAFEPOINT
{
#if defined(GC_VERIFY) || defined(gc_assert_obj_validity)
    (void)parent;
    gc_assert_obj_validity_(child);
#endif
}
...
JL_DLLEXPORT void jl_gc_log_field_write(int64_t parent_addr,int64_t child_addr)
{
    ...
    gc_assert_obj_validity_(parent);
    gc_assert_obj_validity_(child);
}
                         ...
                         uint8_t *obj8_end, uintptr_t nptr) JL_NOTSAFEPOINT
{
    ...
    for (; obj8_begin < obj8_end; obj8_begin++) {
        ...
        if (new_obj != NULL) {
            ...
            gc_assert_obj_validity((jl_value_t *)obj8_parent, new_obj);
                          ...
                          uint16_t *obj16_end, uintptr_t nptr) JL_NOTSAFEPOINT
{
    ...
    for (; obj16_begin < obj16_end; obj16_begin++) {
        ...
        if (new_obj != NULL) {
            ...
            gc_assert_obj_validity((jl_value_t *)obj16_parent, new_obj);
                          ...
                          uint32_t *obj32_end, uintptr_t nptr) JL_NOTSAFEPOINT
{
    ...
    for (; obj32_begin < obj32_end; obj32_begin++) {
        ...
        if (new_obj != NULL) {
            ...
            gc_assert_obj_validity((jl_value_t *)obj32_parent, new_obj);
                      ...
                      jl_value_t **obj_end, uint32_t step, uintptr_t nptr) JL_NOTSAFEPOINT
{
    ...
    for (; obj_begin < scan_end; obj_begin += step) {
        ...
        if (new_obj != NULL) {
            verify_parent2("obj array", obj_parent, obj_begin, "elem(%d)",
                        gc_slot_to_arrayidx(obj_parent, obj_begin));
            gc_assert_obj_validity(obj_parent, new_obj);
            gc_try_claim_and_push(mq, new_obj, &nptr);
            gc_heap_snapshot_record_array_edge(obj_parent, slot);
        }
    }
    ...
    for (; ary8_begin < scan_end; ary8_begin += elsize) {
        for (uint8_t *pindex = elem_begin; pindex < elem_end; pindex++) {
            ...
            if (new_obj != NULL) {
                verify_parent2("array", ary8_parent, &new_obj, "elem(%d)",
                               gc_slot_to_arrayidx(ary8_parent, ary8_begin));
                gc_assert_obj_validity(ary8_parent, new_obj);
                gc_try_claim_and_push(mq, new_obj, &nptr);
                gc_heap_snapshot_record_array_edge(ary8_parent, slot);
            }
        }
    ...
    for (; ary16_begin < scan_end; ary16_begin += elsize) {
        for (uint16_t *pindex = elem_begin; pindex < elem_end; pindex++) {
            ...
            if (new_obj != NULL) {
                verify_parent2("array", ary16_parent, &new_obj, "elem(%d)",
                               gc_slot_to_arrayidx(ary16_parent, ary16_begin));
                gc_assert_obj_validity(ary16_parent, new_obj);
                gc_try_claim_and_push(mq, new_obj, &nptr);
                gc_heap_snapshot_record_array_edge(ary16_parent, slot);
            }
        }
                            ...
                            uint8_t bits) JL_NOTSAFEPOINT
{
    ...
    gc_assert_obj_validity((jl_value_t *)mb_parent, (jl_value_t *)mb_parent->scanned_methods);
