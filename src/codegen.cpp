...
#include <thread>
thread_local uint64_t AA_NOOP = 0;

static void visitLine(jl_codectx_t &ctx, uint64_t *ptr, Value *addend, const char *name)
{
    Value *AllocaAsInt64 = ConstantInt::get(ctx.types().T_size, (uintptr_t)&AA_NOOP);

    Value *ptr_as_integer = ConstantInt::get(ctx.types().T_size, (uintptr_t)ptr);
    ...
    Value *new_value = ctx.builder.CreateAdd(v, addend);

    // Cast the boolean condition to an integer type
    Value *condition_as_int = ctx.builder.CreateZExt(is_equal_zero, getInt64Ty(ctx.builder.getContext()));
